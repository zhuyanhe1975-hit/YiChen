      ...
      <div className="flex flex-col items-center justify-center h-[80%] relative z-10 space-y-8">
        ...
        <div className="flex gap-4">
          {data.parents.map((p, i) => (
            <div key={i} className="bg-blue-600/80 px-4 py-2 rounded-lg border border-blue-400 text-sm shadow-lg shadow-blue-500/20 backdrop-blur-sm">
              ⬆️ {p}
            </div>
          ))}
        </div>
        ...
        <div className="relative">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-red-600 w-40 h-40 rounded-full flex items-center justify-center text-center p-4 border-4 border-yellow-400 shadow-2xl relative z-10 font-bold text-lg transform hover:scale-110 transition-transform duration-300 cursor-default">
            {data.center}
            </div>
        </div>
            ...
            <div className="flex flex-col items-center gap-2">
                 ...
                 <div className="flex gap-3 flex-wrap justify-center">
                    {data.children.map((c, i) => (
                        <div key={i} className="bg-green-600/80 px-4 py-2 rounded-lg border border-green-400 text-sm shadow-lg backdrop-blur-sm">
                        ⬇️ {c}
                        </div>
                    ))}
                 </div>
            ...
            {data.related.length > 0 && (
                <div className="flex flex-col items-center gap-2 border-l border-white/20 pl-8">
                    ...
                    <div className="flex gap-3 flex-wrap justify-center">
                        ...
                        <div key={i} className="bg-purple-600/80 px-4 py-2 rounded-lg border border-purple-400 text-sm shadow-lg backdrop-blur-sm">
                            🔗 {r}
                        </div>
                        ))}
                    </div>
